...
and the \textit{inter-collection lag} between our two collector nodes (\autoref{fig:inter-collection-lag}). 
...
Finally, acknowledging that the \gls{ln} is a distributed and not strictly decentralized network, 
...
as well as how good it actually spreads.
...
We first begin by showing the number of gossip messages in the time frame to estimate the baseline network volume. 
To assess propagation consistency, we evaluate the intersection of messages received by our two independent collector nodes (\textit{Alice} and \textit{Bob}).

\subsubsection{Network Gossip Volume and Overlap}

Both collector nodes (\textit{Alice} and \textit{Bob}) operate independently and 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{resources/results-final/network-gossip-volume-and-overlap.pdf}
    \caption {
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{resources/results-final/peer-session-distribution.pdf}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{resources/results-final/corelation-node-size-and-session-duration.pdf}
    \caption{
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{resources/results-final/gossip-distribution-by-type.pdf}
\caption{
    ...
    Notably, during the intermediate collection gap, primarily only \code{channel\_announcements} were consistently preserved. 
    Under active monitoring conditions, \code{channel\_updates} dominate the network broadcast volume, 
    whereas structural \code{channel\_announcements} remain the least frequent.
}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{resources/results-final/channel-lifetime-distribution.pdf}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{resources/results-final/channel-capacity-lifetime-correlation.pdf}
    \caption{
