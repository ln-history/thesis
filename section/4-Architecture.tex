...
followed by a high-level architectural overview in \autoref{sec:high-level-architecture}.
...
real-time analysis and \autoref{sec:batch-pipeline} for bulk insertion, t
o the storage layer in \autoref{sec:database} and its two auxiliary services:
...
The chapter comes to an end by first discussing continuous monitoring of the system in \autoref{sec:observability}.
Lastly, the \autoref{sec:deployment} focuses on deployment.
...
The \term{time-machine} tool iterates through a file of gossip messages and yields a \tech{networkx} graph~\cite{hagberg_exploring_2008}.
This, simple yet effective procedure takes approximately 10 minutes on commodity hardware (e.g., Apple M1, 16 GB RAM) 
to process ten million gossip messages and yields a single graph for a single timestamp.
This means, that the creation of $n$ snapshots requires $n$ iterations of the \term{time-machine} tool
...
\begin{enumerate}[label=\textbf{RF\arabic*}, ref=RF\arabic*, resume, leftmargin=3.5em]
    ...
    \item \label{req:snapshot-diff} \textbf{Time-Window Retrieval:} 
        ...
        broadcasted between two given timestamps $t_{start}$ and $t_{end}$.
    \item \label{req:entity-filtering} \textbf{Entity Filtering:} 
        The system must be able to filter the gossip history by identifiers, 
        specifically \code{node\_id} and \code{scid}.
\end{enumerate}

\subsubsection{Real-Time Operations}
\begin{enumerate}[label=\textbf{RF\arabic*}, ref=RF\arabic*, resume, leftmargin=3.5em]
    \item \label{req:ingestion} \textbf{Low-Latency Ingestion:} 
        Newly received gossip from the \gls{p2p} network must be inserted 
    ...
    \item \label{req:multiple-nodes} \textbf{Multiple nodes:} 
        The system must be scalable to multiple collector nodes that insert 
    ...
    \item \label{req:metadata}\textbf{Metadata:} 
        ...
        must be linked to the gossip message and must also be stored.
...
\begin{enumerate}[label=\textbf{RN\arabic*}, ref=RN\arabic*, leftmargin=3.5em]
    ...
    This ensures the platform is environment-agnostic, allowing it to run consistently on different machines.
    ...
    By lowering the barrier to entry to host \tech{ln-history}.
...
As batch insertions of historic gossip messages rarely occur, 
...
Lastly, the \service{ln-history-api} queries the \tech{ln-history-database} 
...
The byte layout of an individual record is shown in \autoref{fig:core-lightning-gossip-store-bytefield}.
...
For each \code{channel\_announcement} the foreign key constraints are checked by the \service{gossip-processor} 
...
\section{Batch Ingestion Pipeline}
...
The script tracks the \code{first\_seen} and \code{last\_seen} timestamps during the extraction
...
the script utilizes a configured environment variable (\code{HISTORIC\_COLLECTOR\_NODE\_ID}) to inject dummy collector node values. 
...
The centre of the data model, see \autoref{fig:db-master-registry} begins with 
...
and the \code{channel\_announcements} table utilizes the \code{idx\_chan\_val\-id\-ity} on \code{(funding\_timestamp, closing\_timestamp)}.
...
\subsection{Blockchain Data Enrichment: \service{chain-enricher}}
...
The motivation for the \service{chain-enricher} is to separate the gathering of on-chain information from the \service{gossip-processor}. 
...
If \code{bitcoin\_key\_1} and \code{bitcoin\_key\_2} are present in the channel's gossip data, 
...
As elaborated in \autoref{sec:gossip-messages}, the \qty{64}{\bit} \gls{scid} deterministically encodes the block height, transaction index, and output index. 
...
\begin{listing}[htbp]
    ...
    \label{lst:get-snapshot-at-timestamp-header}
\end{listing}

As shown in \autoref{lst:get-snapshot-at-timestamp-header}, the \code{get\_snapshot\_at\_timestamp} method 
