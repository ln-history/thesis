...
Compounding these synchronization inefficiencies is the suboptimal utilization of node bandwidth. 
...
it is evident that researching and implementing optimizations to the gossip protocol is necessary for long-term scalability. 
...
The analysis demonstrated that peer session durations exhibit immense variance, ranging from minutes to weeks. 
...
By ensuring uninterrupted access to real-time topological updates and fee policies, 
...
The evolution of the \gls{ln} is a complex topic and can be analysed from different of angles.
As we will see, the network has matured in the past eight years.
Nonetheless, there still many open questions and possible improvements to be made.
...
After that we will, examine empirical measurements of channels.
...
while structural \code{channel\_announcements} represent only a marginal fraction of the gossip protocol.
...
the empirical reality is different: of all channels that have ever been funded, more than \qty{75}{\percent}

It is reasonable to assume that larger channels might have a longer lifespan than smaller ones. 
Node operators committing significant financial capital to open large channels 
...
The closure rate of over \qty{80}{\percent}, considering the networks whole history, can arguably be considered high.
Combined with the lack of correlation to capacity—indicates that channels frequently require active adjustments, closures, and rebalancing. 
...
Professional routing hubs provide vital liquidity and continuous uptime necessary for a functional payment network. 
...
Enforcing strict, uniform decentralization would likely fragment liquidity and increase routing failures.

Nonetheless, from the perspectives of privacy, censorship resistance, and systemic independence, 
this centralization of routing power remains highly concerning. 
Relying on a handful of dominant hub operators directly contradicts the foundational, 
\gls{p2p} ethos of the Bitcoin ecosystem. 
These central hubs represent a massive vulnerability regarding user privacy. 
...
dominant routing nodes could theoretically trace and deanonymize payment flows, 
severely degrading the privacy guarantees originally engineered into the protocol~\cite{pickhardt_security_2021}.
